...
def selection_sort_steps(arr):
    ...
    for i in range(n):
        ...
        for j in range(i + 1, n):
            yield Step(list(arr), compare=(min_idx, j), sorted_idx=frozenset(sorted_idx),
                       caption=f"Scan for minimum: compare {min_idx} and {j}")
            if arr[j] < arr[min_idx]:
                min_idx = j
        if min_idx != i:
            arr[i], arr[min_idx] = arr[min_idx], arr[i]
            yield Step(list(arr), swap=(i, min_idx), sorted_idx=frozenset(sorted_idx),
                       caption=f"Swap minimum into place: {i} <-> {min_idx}")
        sorted_idx.add(i)
        yield Step(list(arr), sorted_idx=frozenset(sorted_idx), caption=f"Index {i} finalized")
...
def bubble_sort_steps(arr):
    ...
    for i in range(n):
        ...
        for j in range(0, n - i - 1):
            yield Step(list(arr), compare=(j, j + 1), sorted_idx=frozenset(sorted_idx),
                       caption=f"Compare adjacent pair {j}, {j + 1}")
            if arr[j] > arr[j + 1]:
                arr[j], arr[j + 1] = arr[j + 1], arr[j]
                swapped = True
                yield Step(list(arr), swap=(j, j + 1), sorted_idx=frozenset(sorted_idx),
                           caption=f"Swap {j}, {j + 1}")
        sorted_idx.add(n - i - 1)
        yield Step(list(arr), sorted_idx=frozenset(sorted_idx), caption=f"Index {n - i - 1} finalized")
        if not swapped:
            break
...
def insertion_sort_steps(arr):
    n = len(arr)
    yield Step(list(arr), sorted_idx=frozenset({0}), caption="Index 0 is trivially sorted")
    for i in range(1, n):
        key = arr[i]
        j = i - 1
        ...
        while j >= 0 and arr[j] > key:
            ...
            j -= 1
            if j >= 0:
                yield Step(list(arr), compare=(j, i), caption="Compare key against next")
        arr[j + 1] = key
        yield Step(list(arr), write=(j + 1,), sorted_idx=frozenset(range(i + 1)),
...
def merge_sort_steps(arr):
    def merge_sort(l, r):
        ...
        yield Step(list(arr), aux={"type": "segment", "range": (l, r)}, caption=f"Merging [{l}, {r})")
        left, right = arr[l:m], arr[m:r]
        i = j = 0
        k = l
        while i < len(left) and j < len(right):
            yield Step(list(arr), compare=(l + i, m + j), aux={"type": "segment", "range": (l, r)},
            ...
            if left[i] <= right[j]:
                ...
                arr[k] = right[j]
                j += 1
            yield Step(list(arr), write=(k,), aux={"type": "segment", "range": (l, r)}, caption=f"Write index {k}")
            k += 1
        while i < len(left):
            arr[k] = left[i]
            i += 1
            yield Step(list(arr), write=(k,), aux={"type": "segment", "range": (l, r)}, caption="Flush left run")
            k += 1
        while j < len(right):
            arr[k] = right[j]
            j += 1
            yield Step(list(arr), write=(k,), aux={"type": "segment", "range": (l, r)}, caption="Flush right run")
            k += 1
...
def quick_sort_steps(arr):
    def qsort(lo, hi, sorted_idx):
        ...
        for j in range(lo, hi):
            yield Step(list(arr), compare=(j, hi), aux={"type": "pivot", "idx": hi}, sorted_idx=frozenset(sorted_idx),
                       caption=f"Compare {j} against pivot")
            if arr[j] <= pivot:
                i += 1
                if i != j:
                    arr[i], arr[j] = arr[j], arr[i]
                    yield Step(list(arr), swap=(i, j), aux={"type": "pivot", "idx": hi},
                               sorted_idx=frozenset(sorted_idx), caption=f"Swap {i}, {j}")
        arr[i + 1], arr[hi] = arr[hi], arr[i + 1]
        yield Step(list(arr), swap=(i + 1, hi), sorted_idx=frozenset(sorted_idx), caption=f"Pivot placed at {i + 1}")
        sorted_idx.add(i + 1)
...
def heap_sort_steps(arr):
    ...
    def sift_down(start, end):
        ...
        while True:
            ...
            if child > end:
                break
            yield Step(list(arr), compare=(root, child), aux={"type": "heap", "size": end + 1},
                       caption="Compare parent with left child")
            if child + 1 <= end:
                yield Step(list(arr), compare=(child, child + 1), aux={"type": "heap", "size": end + 1},
                           caption="Compare children")
                if arr[child + 1] > arr[child]:
                    child += 1
            if arr[root] < arr[child]:
                arr[root], arr[child] = arr[child], arr[root]
                yield Step(list(arr), swap=(root, child), aux={"type": "heap", "size": end + 1},
    ...
    for end in range(n - 1, 0, -1):
        ...
        sorted_idx.add(end)
        yield Step(list(arr), swap=(0, end), aux={"type": "heap", "size": end}, sorted_idx=frozenset(sorted_idx),
...
def cycle_sort_steps(arr):
    ...
    for cycle_start in range(n - 1):
        ...
        for i in range(cycle_start + 1, n):
            yield Step(list(arr), compare=(cycle_start, i), aux={"type": "cycle", "start": cycle_start},
        ...
        if pos != cycle_start:
            ...
            yield Step(list(arr), write=(pos,), aux={"type": "cycle", "start": cycle_start},
...
def three_way_merge_sort_steps(arr):
    def merge3(l, r):
        ...
        yield Step(list(arr), aux={"type": "segment", "range": (l, r)}, caption=f"3-way merging [{l}, {r})")
        a, b, c = arr[l:m1], arr[m1:m2], arr[m2:r]
        i = j = k = 0
        idx = l
        while i < len(a) or j < len(b) or k < len(c):
            yield Step(list(arr), aux={"type": "segment", "range": (l, r)}, caption="Compare heads of 3 runs")
            candidates = []
            if i < len(a):
            ...
            if which == "a":
                ...
                arr[idx] = c[k]
                k += 1
            yield Step(list(arr), write=(idx,), aux={"type": "segment", "range": (l, r)}, caption=f"Write index {idx}")
            idx += 1
...
def counting_sort_steps(arr):
    ...
    for v in arr:
        count[v - lo] += 1
        yield Step(list(arr), aux={"type": "count", "table": list(count), "lo": lo}, caption=f"Tally value {v}")
    for i in range(1, len(count)):
        count[i] += count[i - 1]
        yield Step(list(arr), aux={"type": "count", "table": list(count), "lo": lo, "prefix": True},
                   caption="Build prefix-sum (placement) table")
    output = [None] * n
    for v in reversed(arr):
        count[v - lo] -= 1
        output[count[v - lo]] = v
        yield Step(list(output), aux={"type": "count", "table": list(count), "lo": lo},
...
def radix_sort_steps(arr):
    ...
    while max_val // exp > 0:
        ...
        for v in arr:
            d = (v // exp) % 10
            buckets[d].append(v)
            yield Step(list(arr), aux={"type": "radix", "buckets": [list(b) for b in buckets], "digit": d},
                       caption=f"Pass {pass_num}: digit {d} of {v} (place value {exp})")
        arr[:] = [v for bucket in buckets for v in bucket]
        yield Step(list(arr), aux={"type": "radix", "buckets": [list(b) for b in buckets]},
...
def bucket_sort_steps(arr):
    ...
    for v in arr:
        ...
        yield Step(list(arr), aux={"type": "buckets", "buckets": [list(b) for b in buckets]},
    ...
    for b in buckets:
        for i in range(1, len(b)):
            ...
            while j >= 0 and b[j] > key:
                ...
                j -= 1
            b[j + 1] = key
        yield Step(list(arr), aux={"type": "buckets", "buckets": [list(x) for x in buckets]},
                   caption="Insertion-sort within each bucket")
    arr[:] = [v for bucket in buckets for v in bucket]
    yield Step(list(arr), sorted_idx=frozenset(range(n)), aux={"type": "buckets", "buckets": [list(x) for x in buckets]},
...
def pigeonhole_sort_steps(arr):
    ...
    for v in arr:
        holes[v - lo].append(v)
        yield Step(list(arr), aux={"type": "holes", "holes": [list(h) for h in holes], "lo": lo},
                   caption=f"Drop {v} directly into hole {v}")
    arr[:] = [v for hole in holes for v in hole]
    yield Step(list(arr), sorted_idx=frozenset(range(n)), aux={"type": "holes", "holes": [list(h) for h in holes], "lo": lo},
...
def introsort_steps(arr):
    ...
    def ins_sort(lo, hi):
        yield Step(list(arr), aux={"type": "mode", "mode": "insertion"}, caption=f"Range too small [{lo},{hi}] -> Insertion Sort")
        for i in range(lo + 1, hi + 1):
            key = arr[i]
            j = i - 1
            while j >= lo and arr[j] > key:
                arr[j + 1] = arr[j]
                yield Step(list(arr), write=(j + 1,), aux={"type": "mode", "mode": "insertion"}, caption="Shift")
                j -= 1
            arr[j + 1] = key
            yield Step(list(arr), write=(j + 1,), aux={"type": "mode", "mode": "insertion"}, caption="Insert")

    def heap_range(lo, hi):
        yield Step(list(arr), aux={"type": "mode", "mode": "heap"}, caption=f"Depth limit hit on [{lo},{hi}] -> Heap Sort")
        sub = arr[lo:hi + 1]
        for step in heap_sort_steps(sub):
            arr[lo:hi + 1] = sub
            remap = lambda idxs: tuple(x + lo for x in idxs) if idxs else None
            yield Step(list(arr), compare=remap(step.compare), swap=remap(step.swap), write=remap(step.write),
                       aux={"type": "mode", "mode": "heap"}, caption=step.caption)
    ...
    def qsort(lo, hi, depth):
        while hi - lo + 1 > size_threshold:
            if depth <= 0:
                ...
                return
            depth -= 1
            yield Step(list(arr), aux={"type": "mode", "mode": "quick"}, caption=f"Quicksort partition [{lo},{hi}]")
            pivot = arr[hi]
            i = lo - 1
            for j in range(lo, hi):
                yield Step(list(arr), compare=(j, hi), aux={"type": "mode", "mode": "quick"}, caption="Compare to pivot")
                if arr[j] <= pivot:
                    i += 1
                    if i != j:
                        arr[i], arr[j] = arr[j], arr[i]
                        yield Step(list(arr), swap=(i, j), aux={"type": "mode", "mode": "quick"}, caption="Swap")
            arr[i + 1], arr[hi] = arr[hi], arr[i + 1]
            yield Step(list(arr), swap=(i + 1, hi), aux={"type": "mode", "mode": "quick"}, caption="Pivot placed")
            p = i + 1
            if p - lo < hi - p:
...
def timsort_steps(arr):
    ...
    while i < n:
        ...
        yield Step(list(arr), aux={"type": "runs", "runs": runs + [(i, run_end)]},
        ...
        for k in range(i + 1, run_end):
            ...
            while p >= i and arr[p] > key:
                arr[p + 1] = arr[p]
                yield Step(list(arr), write=(p + 1,), aux={"type": "runs", "runs": runs + [(i, run_end)]},
        ...
        i = run_end
    yield Step(list(arr), aux={"type": "runs", "runs": list(runs)}, caption=f"Identified {len(runs)} natural runs")
    ...
    while len(runs) > 1:
        ...
        for a in it:
            ...
            if b is None:
                ...
                continue
            l, m, r = a[0], a[1], b[1]
            yield Step(list(arr), aux={"type": "segment", "range": (l, r)}, caption=f"Merging runs [{l},{m}) + [{m},{r})")
            left, right = arr[l:m], arr[m:r]
            x = y = 0
            k = l
            while x < len(left) and y < len(right):
                yield Step(list(arr), compare=(l + x, m + y), aux={"type": "segment", "range": (l, r)},
                           caption="Compare run heads")
                if left[x] <= right[y]:
                    arr[k] = left[x]
                    x += 1
                    ...
                    arr[k] = right[y]
                    y += 1
                yield Step(list(arr), write=(k,), aux={"type": "segment", "range": (l, r)}, caption=f"Write {k}")
                k += 1
            while x < len(left):
                arr[k] = left[x]
                x += 1
                yield Step(list(arr), write=(k,), aux={"type": "segment", "range": (l, r)}, caption="Flush left")
                k += 1
            while y < len(right):
                arr[k] = right[y]
                y += 1
                yield Step(list(arr), write=(k,), aux={"type": "segment", "range": (l, r)}, caption="Flush right")
                k += 1
            new_runs.append((l, r))
        runs = new_runs
    yield Step(list(arr), sorted_idx=frozenset(range(n)), caption="Sorted")
...
def _self_check():
    ...
    for name, fn in ALGORITHMS.items():
        ...
        assert list(last.array) == expected, f"{name}: final array {last.array} != {expected}"
